...
StringPiece LongestCommonSubstring(const StringPiece& str1, const StringPiece str2) {
    if (str1.empty() || str2.empty()) return "";

    int str1_size = static_cast<int>(str1.size());
    int str2_size = static_cast<int>(str2.size());

    int common_sub_string_str1_pos = -1;
    int common_sub_string_str2_pos = -1;

    int longest = 0;

    int indices[] = {0, 0};
    int sizes[] = {str1_size, str2_size};
    ...
    for (int index = 0; index < 2; ++index) {
        ...
        for (i = index; i < size; ++i) {
            ...
            if (((str1_size - m) <= longest) || ((str2_size - n) <= longest)) break;

            for (; m < str1_size && n < str2_size; ++m, ++n) {
                if (str1[m] != str2[n]) {
                    length = 0;
                } else {
                    ++length;
                    if (longest < length) {
                        longest = length;
                        common_sub_string_str1_pos = m - longest + 1;
                        common_sub_string_str2_pos = n - longest + 1;
                    }
                }
            }
        }
    ...
    if (0 == longest) {
        ...
        return str1.substr(common_sub_string_str1_pos, longest);
    }
}
} // namespace toft
