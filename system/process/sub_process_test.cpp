...
TEST_F(SubProcessTest, CloseChildFd)
{
    SubProcess::CreateOptions options;
    options.CloseFds();
    SubProcess process;
    ...
    ASSERT_TRUE(process.Create(m_exe_path + " " + m_exe_args, options));
    pid_t child_pid = process.Id();
    std::list<int> fds;
    ASSERT_TRUE(process.GetOpenFds(child_pid, &fds));
    // 4 : stdin, stdout, stderr, pipe_write_fd
    EXPECT_LE(fds.size(), 4u);
    ASSERT_TRUE(process.TimedWaitForExit(6000));
    EXPECT_EQ(0, process.ExitCode());
    EXPECT_FALSE(process.IsValid());
}
