...
int64_t MockFileSystem::GetSize(const std::string& file_path) {
    CHECK_FILE_SYSTEM_MOCK();
    return s_mock->GetSize(file_path);
}
...
bool MockFileSystem::Mkdir(const std::string& dir, int mode) {
    CHECK_FILE_SYSTEM_MOCK();
    return s_mock->Mkdir(dir, mode);
}

bool MockFileSystem::Rmdir(const std::string& dir) {
    CHECK_FILE_SYSTEM_MOCK();
    return s_mock->Rmdir(dir);
}
