...
bool LocalSequenceFileReader::RealSeek(int64_t offset) {
    return m_data_input->Seek2(offset);
}
...
int LocalSequenceFileReader::ReadRecordWithMaxSize(std::string* key,
                                                   std::string* value,
                                                   int max_size) {
    StringPiece key_piece, value_piece;
    int ret_code = ReadRecordWithMaxSize(&key_piece, &value_piece, max_size);
    if (READ_SUCCESS != ret_code) {
        return ret_code;
    }
...
int LocalSequenceFileReader::ReadRecordWithMaxSize(StringPiece* key,
                                                   StringPiece* value,
                                                   int max_size) {
    if (!m_is_head_parsed) {
        if (!LoadSequenceFileHeader()) {
    ...
    if (record_len > max_size) {
        LOG(WARNING) << "record too long. record_len:" << record_len << " max_size:" << max_size;
        return READ_TOO_LONG_MESSAGE;
    }
    ...
    if (key_len < 0 || key_len > max_size || value_len < 0 || value_len > max_size) {
        LOG(WARNING) << "bad record. key_len:" << key_len << " value_len:" << value_len;
        return READ_TOO_LONG_MESSAGE;
    }

    if (!m_data_input->ReadKeyValue(key_len, value_len, key, value)) {
        return READ_FAILED;
    }
    return READ_SUCCESS;
}
