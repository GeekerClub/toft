...
public:
    ...
    // seek to offset directly
    virtual bool RealSeek(int64_t offset);
    ...
    enum ReadRecordWithMaxSizeRetCode {
        READ_SUCCESS = 0,
        READ_FAILED = 1,
        READ_TOO_LONG_MESSAGE = 2,
    };

    virtual int ReadRecordWithMaxSize(std::string* key, std::string* value, int max_size);

    virtual int ReadRecordWithMaxSize(StringPiece* key, StringPiece* value, int max_size);
