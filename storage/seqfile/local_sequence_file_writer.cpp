...
bool LocalSequenceFileWriter::Write(const char* addr, int len) {
    return (m_file->Write(addr, len) == len) && m_file->Flush();
    // int64_t writed_bytes = m_file->Write(addr, len);
    // while (len != writed_bytes && len >= 0) {
    //     VLOG(40) << "len is " << len << " and writed_bytes is " << writed_bytes;
    //     addr += writed_bytes;
    //     len -= writed_bytes;
    //     writed_bytes = m_file->Write(addr, len);
    // }
    // return m_file->Flush();
}
