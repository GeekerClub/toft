...
public:
    ...
    bool Seek2(int64_t offset) {
        // when tailing
        int64_t file_offset = m_file->Tell();
        int64_t buffer_start_offset = file_offset - m_size;
        if (buffer_start_offset <= offset && offset < file_offset) {
            m_offset = offset - buffer_start_offset;
            return true;
        }
